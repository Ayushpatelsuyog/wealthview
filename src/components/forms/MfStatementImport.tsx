    ...
    <div className="space-y-4">
        ...
        {[1, 2, 3, 4].map(s => (
          <div key={s} className="flex items-center gap-1">
            <div className="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold"
              style={{
                backgroundColor: step >= s ? '#1B2A4A' : 'var(--wv-border)',
                color: step >= s ? 'white' : '#9CA3AF',
              }}>
              {step > s ? '\u2713' : s}
            </div>
            {s < 4 && <div className="w-8 h-0.5" style={{ backgroundColor: step > s ? '#1B2A4A' : 'var(--wv-border)' }} />}
          </div>
        ))}
        <span className="text-[10px] ml-2" style={{ color: 'var(--wv-text-muted)' }}>
      ...
      {step === 1 && (
        <div className="space-y-3">
          <div className="border-2 border-dashed rounded-xl p-6 text-center transition-colors"
            style={{ borderColor: files.length > 0 ? '#C9A84C' : 'var(--wv-border)' }}>
            {files.length > 0 ? (
              <div className="space-y-2">
                {files.map((f, i) => (
                  <div key={i} className="flex items-center justify-between px-3 py-1.5 rounded-lg" style={{ backgroundColor: 'var(--wv-surface-2)' }}>
                    <div className="flex items-center gap-2">
                      <FileText className="w-3.5 h-3.5" style={{ color: '#C9A84C' }} />
                      <span className="text-xs font-medium" style={{ color: 'var(--wv-text)' }}>{f.name}</span>
                      <span className="text-[10px]" style={{ color: 'var(--wv-text-muted)' }}>({Math.round(f.size / 1024)}KB)</span>
                    </div>
                    <button onClick={() => removeFile(i)}><X className="w-3 h-3" style={{ color: 'var(--wv-text-muted)' }} /></button>
                  </div>
                ))}
              ...
              <label className="cursor-pointer">
                ...
                <Upload className="w-8 h-8 mx-auto mb-2" style={{ color: 'var(--wv-text-muted)' }} />
                ...
                <p className="text-[10px] mt-1" style={{ color: 'var(--wv-text-muted)' }}>Supports HDFC, SBI, ICICI, Axis, Mirae, and other AMC formats</p>
              </label>
            )}
          </div>
      ...
      {step === 2 && (
        <div className="space-y-3">
          ...
          <div className="p-3 rounded-lg text-xs" style={{ backgroundColor: 'rgba(27,42,74,0.04)', border: '1px solid rgba(27,42,74,0.08)' }}>
            <p className="font-semibold mb-1" style={{ color: 'var(--wv-text)' }}>
          ...
          {parsedFunds.map((fund, fi) => (
            <div key={fi} className="border rounded-lg" style={{ borderColor: fund.selected ? '#C9A84C' : 'var(--wv-border)', opacity: fund.selected ? 1 : 0.6, position: 'relative' }}>
              {/* Fund header */}
              <div className="px-3 py-2" style={{ backgroundColor: fund.selected ? 'rgba(201,168,76,0.06)' : '#F7F5F0' }}>
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold truncate" style={{ color: 'var(--wv-text)' }}>{fund.fundName}</p>
                  ...
                  <button onClick={() => updateFund(fi, { expanded: !fund.expanded })} className="flex-shrink-0">
                    {fund.expanded ? <ChevronUp className="w-4 h-4" style={{ color: 'var(--wv-text-muted)' }} /> : <ChevronDown className="w-4 h-4" style={{ color: 'var(--wv-text-muted)' }} />}
                  </button>
                </div>
                ...
                {fund.selected && (
                  <div className="space-y-2 mt-2">
                    <div className="grid grid-cols-3 gap-2">
                      <div>
                        <Label className="text-[9px]" style={{ color: 'var(--wv-text-muted)' }}>Member</Label>
                      ...
                      <div>
                        <Label className="text-[9px]" style={{ color: 'var(--wv-text-muted)' }}>Portfolio</Label>
                      ...
                      <div>
                        <Label className="text-[9px]" style={{ color: 'var(--wv-text-muted)' }}>Distributor {!fund.brokerId && <span style={{ color: '#DC2626' }}>*</span>}</Label>
                    ...
                    <div>
                      <Label className="text-[9px]" style={{ color: 'var(--wv-text-muted)' }}>AMFI Scheme</Label>
                      {fund.matched && !fund.editingScheme ? (
                        ...
                        <div className="relative">
                          ...
                          {(schemeSearchResults[fi] || []).length > 0 && (
                            <div className="absolute top-full left-0 right-0 mt-1 border rounded-lg bg-white shadow-lg max-h-40 overflow-y-auto" style={{ borderColor: 'var(--wv-border)', zIndex: 9999 }}>
                              {(schemeSearchResults[fi] || []).map((r: any, ri: number) => (
                                <button key={ri}
                                  ...
                                  <p className="font-medium" style={{ color: 'var(--wv-text)' }}>{r.schemeName}</p>
                                  <p style={{ color: 'var(--wv-text-muted)' }}>Code: {r.schemeCode}</p>
                                </button>
                              ))}
                            </div>
                          )}
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr style={{ backgroundColor: '#FAFAF8' }}>
                        <th className="px-2 py-1 text-left font-medium" style={{ color: 'var(--wv-text-muted)' }}>Date</th>
                        <th className="px-2 py-1 text-left font-medium" style={{ color: 'var(--wv-text-muted)' }}>Type</th>
                        <th className="px-2 py-1 text-right font-medium" style={{ color: 'var(--wv-text-muted)' }}>Amount</th>
                        <th className="px-2 py-1 text-right font-medium" style={{ color: 'var(--wv-text-muted)' }}>NAV</th>
                        <th className="px-2 py-1 text-right font-medium" style={{ color: 'var(--wv-text-muted)' }}>Units</th>
                      </tr>
                    </thead>
                    <tbody>
                      {fund.transactions.map((txn: any, ti: number) => {
                        ...
                        return (
                          <tr key={ti} style={{ borderTop: '1px solid #F0EDE6' }}>
                            <td className="px-2 py-1" style={{ color: 'var(--wv-text)' }}>{txn.date ?? '\u2014'}</td>
                            <td className="px-2 py-1" style={{ color: 'var(--wv-text-secondary)' }}>{txn.type ?? '\u2014'}</td>
      ...
      {step === 3 && (
        <div className="p-8 text-center">
          ...
          <p className="text-sm font-medium" style={{ color: 'var(--wv-text)' }}>{importProgress || 'Importing...'}</p>
        </div>
      )}
      ...
      {step === 4 && importResult && (
        <div className="p-6 text-center">
          ...
          <p className="text-lg font-semibold mb-1" style={{ color: 'var(--wv-text)' }}>Import Complete</p>
          <p className="text-sm mb-4" style={{ color: 'var(--wv-text-secondary)' }}>
