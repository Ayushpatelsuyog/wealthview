...
export async function POST(req: NextRequest) {
  ...
  const { txn_id, holding_id, quantity, price, date, fees, notes, type } = body;
  ...
  for (const txn of (allTxns ?? [])) {
    ...
    if (txn.type === 'buy' || txn.type === 'sip') {
      if (n.includes('bonus')) {
        // Bonus: qty increases, cost stays same, avg spreads
        totalQty += tQty;
        // Cost doesn't change for bonus shares
      } else {
        totalCost += tQty * tPx;
        totalQty += tQty;
      }
  ...
  console.log(`[Update Txn] Recalc holding ${holding_id}: ${(allTxns ?? []).length} txns, totalQty=${totalQty}, totalCost=${totalCost.toFixed(2)}, newAvg=${avgBuyPrice.toFixed(4)}`);
  ...
  console.log(`[Update Txn] ✓ Holding ${holding_id} updated: qty=${totalQty}, avg=${avgBuyPrice.toFixed(4)}`);
