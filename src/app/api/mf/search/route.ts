...
async function fetchAmfiList(): Promise<void> {
  console.log('[MF Search] Fetching full AMFI scheme list from mfapi.in…');
  ...
  try {
    ...
    if (Array.isArray(data) && data.length > 0) {
      ...
      fetchState = 'done';
      console.log(`[MF Search] ✓ Cached ${amfiList.length} schemes from AMFI API`);
    } else {
    ...
    clearTimeout(tid);
    fetchState = 'failed';
    fetchPromise = null; // allow retry next server restart
    console.error('[MF Search] ✗ AMFI API unavailable:', (err as Error).message, '— will use local list');
  }
}
